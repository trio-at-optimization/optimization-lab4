...
\usepackage{xcolor, ulem, soulutf8, soul, fancyhdr, amsmath, amssymb, amsthm, svg, wrapfig, csvsimple, float, caption, subcaption, titlesec, hyperref, multicol, listings, tocloft, longtable, skak, stmaryrd, color, minted}
...
function $mse\_loss(X_{\text{dataset}}, ~ Y_{\text{dataset}}, ~ w, ~ f)$:
	...
	Для рассмотрения мы возьмем одного представителя из \mintinline{python}|scipy.optimize.minimize| и двух~-- \mintinline{python}|scipy.optimize.least_squares|: L-BFGS, Dog-Box (из Least Squares) и TRF (также оттуда).
